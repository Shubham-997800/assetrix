          ...
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="flex items-center gap-3 rounded-lg border border-border p-3">
              ...
              <div className="min-w-0 flex-1 space-y-1.5">
                <div className="h-3 w-1/2 max-w-[120px] rounded bg-muted" />
                <div className="h-2 w-2/5 max-w-[96px] rounded bg-muted/60" />
              </div>
              <div className="h-5 w-20 flex-shrink-0 rounded-full bg-muted" />
            </div>
          ))}
        </div>
      </div>
