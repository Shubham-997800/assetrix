...
export interface GetAllCyclesParams extends PaginationQuery {
  status?: string;
}
...
export const getAll = async (params: GetAllCyclesParams) => {
  const { page, limit, skip, sortBy, sortOrder } = getPagination(params);

  const where: Prisma.AuditCycleWhereInput = { deletedAt: null };
  if (params.status) where.status = params.status as AuditCycleStatus;

  const searchWhere = buildWhereSearch(["name", "description", "departmentScope"], params.search);
  ...
  return { cycles, meta: paginatedMeta(totalItems, page, limit) };
};
...
export const update = async (id: string, data: UpdateAuditCycleInput, userId: string, ip?: string, userAgent?: string) => {
  ...
  const updateData: Prisma.AuditCycleUpdateInput = {
    ...(data.name !== undefined && { name: data.name }),
    ...(data.description !== undefined && { description: data.description }),
    ...(data.departmentScope !== undefined && { departmentScope: data.departmentScope }),
    ...(data.locationScope !== undefined && { locationScope: data.locationScope }),
...
export const assignAuditors = async (id: string, data: AssignAuditorsInput, userId: string, ip?: string, userAgent?: string) => {
  ...
  await createAuditLog({
    ...
    entity: "AuditCycle",
    entityId: id,
    newValues: { auditorIds: data.auditorIds },
    ipAddress: ip,
    userAgent,
  });
...
export const verifyAsset = async (id: string, data: VerifyAssetInput, userId: string, ip?: string, userAgent?: string) => {
  ...
  type VerificationModel = NonNullable<Awaited<ReturnType<typeof prisma.auditVerification.findFirst>>>;
  let verification: VerificationModel | undefined;
  if (existing) {
...
export const getCycleHistory = async (id: string) => {
  ...
  return { verifications, discrepancies, assignments };
};
