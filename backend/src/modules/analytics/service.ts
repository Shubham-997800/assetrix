...
export const getDashboardStats = async (userId: string) => {
  ...
  const utilizationRate = totalAssetCount > 0
    ? Math.round((activeAllocations / totalAssetCount) * 10000) / 100
    : 0;
  ...
  return {
    ...
    overdueItems: overdueReturns.map((o) => ({
      ...
      days: Math.max(
        1,
        Math.floor((now.getTime() - (o.expectedReturn?.getTime() ?? now.getTime())) / (24 * 60 * 60 * 1000))
      ),
    })),
  };
};
...
export const getAssetAnalytics = async (filters?: {
  ...
  const whereBase: Prisma.AssetWhereInput = { deletedAt: null };
...
export const getMaintenanceAnalytics = async (filters?: {
  ...
  const whereBase: Prisma.MaintenanceTaskWhereInput = { deletedAt: null };
...
export const getFinancialAnalytics = async (filters?: {
  ...
  return {
    overview: {
      ...
      maintenanceTaskCount: maintenanceCosts._count,
    },
...
export const getDepartmentAnalytics = async () => {
  ...
  const departmentAnalytics = departments.map((dept) => {
    ...
    const totalPurchaseValue = dept.assets.reduce((sum, a) => sum + (Number(a.purchasePrice) || 0), 0);
    ...
    const avgAllocationDuration =
      dept.allocations.filter((a) => a.returnedAt).length > 0
        ? dept.allocations
            ...
            .reduce((sum, a) => {
              const duration = new Date(a.returnedAt!).getTime() - new Date(a.allocatedAt).getTime();
