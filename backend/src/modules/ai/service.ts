...
export const getAssetHealthScore = async (assetId: string) => {
  ...
  const warrantyValid = asset.warrantyExpiry && new Date(asset.warrantyExpiry) > now;
...
type StoredRecommendation = Awaited<ReturnType<typeof prisma.aIRecommendation.create>>;

async function generateAndStoreRecommendationsForAsset(assetId: string) {
  const asset = await prisma.asset.findUnique({
    where: { id: assetId },
    include: {
      maintenanceTasks: {
  ...
  const overdueTasks = asset.maintenanceTasks.filter((t) => t.status === "OVERDUE");
  ...
  const recentFailures = asset.maintenanceTasks.filter(
    (t) => t.type === "CORRECTIVE" || t.type === "EMERGENCY"
  );
  ...
  for (const schedule of asset.maintenanceSchedules) {
    if (schedule.nextRunDate && new Date(schedule.nextRunDate) < now) {
      recommendations.push({
        ...
        description: `Scheduled maintenance "${schedule.name}" is overdue (was due ${new Date(
...
export const generateRecommendations = async (userId: string) => {
  ...
  for (const asset of assets) {
    await generateAndStoreRecommendationsForAsset(asset.id);
    generatedCount++;
  }
  ...
  await createActivityLog({
    ...
    description: `Batch recommendation generation completed for ${generatedCount} assets`,
  });

  return {
    jobId: `batch-${Date.now()}`,
    assetCount: generatedCount,
    message: `Recommendations generated synchronously for ${generatedCount} active asset(s). Check the AI recommendations endpoint for results.`,
  };
};

export const getRecommendationStats = async (filters?: { status?: string; type?: string }) => {
  const where: Prisma.AIRecommendationWhereInput = {};
  if (filters?.status) where.status = filters.status;
  if (filters?.type) where.type = filters.type;

  const [byType, byStatus, byPriority, totalCount, recentRecommendations] = await Promise.all([
  ...
  return {
    ...
    byType: byType.map((t) => ({
      type: t.type,
      count: t._count.id,
      avgConfidence: t._avg.confidence ? Math.round(Number(t._avg.confidence) * 100) / 100 : 0,
    })),
...
export const getPredictiveMaintenance = async (assetId?: string) => {
  ...
  const predictions = assets
    .map((asset) => {
      const completedTasks = asset.maintenanceTasks.filter((t) => t.status === "COMPLETED" || true);
      ...
      const avgInterval =
        intervals.length > 0 ? intervals.reduce((a, b) => a + b, 0) / intervals.length : 90;

      const lastMaintenance = completedTasks[0];
      const lastDate = new Date(lastMaintenance.completedAt || lastMaintenance.scheduledDate);
      const predictedNextDate = new Date(lastDate.getTime() + avgInterval * 24 * 60 * 60 * 1000);
      const now = new Date();
      const daysUntil = (predictedNextDate.getTime() - now.getTime()) / (24 * 60 * 60 * 1000);
      ...
      const confidence = Math.max(20, Math.min(95, Math.round((1 - coefficientOfVariation) * 100)));
      ...
      for (const schedule of asset.maintenanceSchedules) {
        if (schedule.nextRunDate) {
          ...
          const schedDays = (schedDate.getTime() - now.getTime()) / (24 * 60 * 60 * 1000);
      ...
      return {
        ...
        historicalData: {
          ...
          lastMaintenanceDate: lastMaintenance.completedAt || lastMaintenance.scheduledDate,
        },
      };
    })
  ...
  const overdueCount = predictions.reduce(
    (sum, p) => sum + p.predictions.filter((pr) => pr.type.includes("OVERDUE")).length,
    0
  );
  const urgentCount = predictions.reduce(
    (sum, p) => sum + p.predictions.filter((pr) => pr.type.includes("UPCOMING")).length,
    0
  );
