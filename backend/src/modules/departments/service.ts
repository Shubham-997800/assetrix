...
import type { CreateDepartmentInput, UpdateDepartmentInput } from "./validators";
...
export const getTree = async (ownerId: string) => {
  ...
  type TreeNode = (typeof departments)[number] & { children: TreeNode[] };
  ...
  for (const dept of departments) {
    ...
    if (!node) continue;
    const parent = dept.parentId ? departmentMap.get(dept.parentId) : undefined;
    if (parent) {
